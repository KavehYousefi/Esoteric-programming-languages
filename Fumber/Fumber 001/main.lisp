...
;; language "Fumber", invented by the Esolang user "PaxtonPenguin" and
;; presented on May 8th, 2024, the kenspeckle signum acquired by this
;; derivative of Urban Mueller's "brainfuck" bifurcating into the
;; twissel of an alternative syntaxis which homologates decimal digits'
;; deployment in lieu of or as a compernage to the octuple brainfuck
;; symbol set, as well as two adscititious commands' introduction.
;; 
;; 
;; Concept
;; =======
...
;; 
;; == FUMBER: BRAINFUCK + DIGIT + MORE ==
;; Fumber's efforts bifurcate into the provision of an alternative
;; denotation for brainfuck's instructions in decimal digits, as well
;; as the utilization of the same character repertoire for two novel
;; actions.
;; 
;; == FUMBER'S SYNTAXIS: DECIMAL DIGITS AS ALTERNATIVE IDENTIFIERS ==
;; The aspect of supererogation to whom accommodated a commorancy in
;; Fumber relates in the paravaunt essay in the twissel potential of
;; the octuple brainfuck competences' expression; imprimis, obeying to
;; the tradition, in the symbolic guise, and secondary, but more peisant
;; in its regard, as a decimal digit.
;; 
;; == DIRECT CELL SETTING AND NO-OPERATIONS ==
;; In addition to the acquainted facilities, the remaining account of
;; decimal digits, "9" and "0", apply themselves to further emolumental
;; exercises, the former a polymechany for the direct equiparation of
;; the current cell's value with a succeeding character's ASCII code,
;; the second a no-operation sentinel.
;; 
...
;; 
;; == THE "9" INSTRUCTION'S CONFORMATION ==
;; The kenspeckle circumstance involving the Fumber instruction "9",
...
;; 
;;   (1) If the argument comprises a symbol affiliated with a native
;;       Fumber instruction, that is, a decimal digit, or a brainfuck
;;       command identifier, the same must be embraced in double
;;       quotation marks.
;;   
...
;; Instructions
;; ============
;; A decimal cardinality's account governs the Fumber programming
;; language, a preponderance of eight members a dation received from
;; its brainfuck cleronomy; augmented, however, by a twissel deviant
;; from the entheus' provenance.
;; 
;; == OVERVIEW ==
;; The twifold contingency for the brainfuck's traditional octuple
;; instruction set to be expressed in a numeric alternative will be
;; administered a particular consideration by the conjugated twain's
;; statement in the "Command" rank, Fumber's poecilonym enjoying the
;; upper echolon's spatiality, while the brainfuck entheus wones in
;; the lower compernage.
;; 
;;   ------------------------------------------------------------------
;;   Command | Effect
;;   --------+---------------------------------------------------------
...
;;   >       | 
;;   ..................................................................
;;   2       | Translates the cell pointer one step to the left.
;;   <       | 
;;   ..................................................................
;;   3       | Increments the current cell value by one (1). If the new
;;   +       | state transgresses the upper bourne of 255, the value
;;           | wraps around to the minimum of zero (0).
;;   ..................................................................
;;   4       | Decrements the current cell value by one (1). If the new
;;   -       | state descends below the lower bourne of zero (0), the
;;           | value wraps around to the maximum of 255.
;;   ..................................................................
;;   5       | Prints the character whose ASCII code corresponds to the
;;   .       | current cell value to the standard output.
;;   ..................................................................
;;   6       | Queries the standard input for a character and stores
;;   ,       | its ASCII code in the current cell.
;;   ..................................................................
;;   7       | If the current cell value equals zero (0), moves the
;;   [       | instruction pointer (IP) forward to the position
;;           | immediately succeeding the matching "8" or "]" token;
;;           | otherwise proceeds as usual.
;;   ..................................................................
;;   8       | If the current cell value does not equal zero (0), moves
;;   ]       | the instruction pointer (IP) back to the position
;;           | immediately succeeding the matching "7" or "[" token;
;;           | otherwise proceeds as usual.
;;   ..................................................................
;;   9       | Consumes the next character in the program and stores
;;           | its ASCII code in the current cell.
;;           |---------------------------------------------------------
...
;;   ..................................................................
;;   0       | Constitutes a no-operation, or nop, that is, accompasses
;;           | no causatum.
;;   ------------------------------------------------------------------
;; 
;; 
...
;; Sources:
;;   [esolang2024Fumber]
;;   The Esolang contributors, "Fumber", May 9th, 2024
;;   URL: "https://esolangs.org/wiki/Fumber"
;; 
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
(defun native-fumber-command-character-p (candidate)
  "Determines whether the CANDIDATE represents a command identifier
   introduced by the Fumber programming language, and absent from the
   brainfuck cleronomy, returning on confirmation a ``boolean'' value of
   ``T'', otherwise ``NIL''."
  (declare (type character candidate))
  (the boolean
    (get-boolean-value-of
...
(defun brainfuck-command-character-p (candidate)
  "Determines whether the CANDIDATE represents an original brainfuck
...
(defun command-character-p (candidate)
  ...
  (declare (type character candidate))
  (the boolean
    (or (native-fumber-command-character-p candidate)
        (brainfuck-command-character-p     candidate))))

;;; -------------------------------------------------------

(defun simple-argument-character-p (candidate)
  "Determines whether the CANDIDATE represents a character admissive in
   its unquoted form as an argument to the Fumber \"9\" instruction,
   returning on confirmation a ``boolean'' value of ``T'', otherwise
   ``NIL''."
  (declare (type character candidate))
  (the boolean
...
(defparameter *source-code*
  "The piece of Fumber source code to analyze.")
...
(defstruct Instruction
  "The ``Instruction'' class serves in the ensconcement of a Fumber
   instruction's requisite information, offered in a Procrustean guise
   as a general compound of a type, an optional argument, and location
   specifiers concerning the provenance source code's occupied tmema."
...
(defclass Program ()
  ...
  (:documentation
    "The ``Program'' class models an executable Fumber program as a
...
(define-condition Fumber-Error (error)
  ()
  (:documentation
    "The ``Fumber-Error'' condition type serves in the communication of
     an anomalous situation whose etiology is ligated to any stage of
     a Fumber program's evaluation."))

;;; -------------------------------------------------------

(define-condition Illegal-Argument-Error (Fumber-Error simple-error)
  ()
  (:documentation
    "The ``Illegal-Argument-Error'' condition type serves in the
     ...
     conformation to the Fumber \"9\" instruction."))

;;; -------------------------------------------------------

(define-condition Missing-Argument-Error (Fumber-Error simple-error)
  ()
  (:documentation
    "The ``Missing-Argument-Error'' condition type serves in the
     communication of an anomalous situation whose etiology derives
     from the attempt to invoke the Fumber \"9\" instruction without
     any succeeding argument."))
...
(defun read-quoted-argument ()
  "Proceeding from the *SOURCE-POSITION*, consumes a quoted argument to
   the impouted previent \"9\" instruction and three values:
...
(defun read-set-to-instruction ()
  "Proceeding from the *SOURCE-POSITION*, evaluates a Fumber \"9\"
      ...
      (cond
        (*source-is-exhausted-p*
          (error 'Missing-Argument-Error
            :format-control
              "Expected an argument to the \"9\" instruction at ~
        ...
        ((native-fumber-command-character-p *source-character*)
          (signal-illegal-argument-error
            "The Fumber command identifier \"~c\", located at ~
             position ~d, must be quoted when used as an ~
             argument."
            *source-character* *source-position*))
        
        ((brainfuck-command-character-p *source-character*)
          (signal-illegal-argument-error
            "The brainfuck command identifier \"~c\", located ~
             at position ~d, must be quoted when used as an ~
             argument."
            *source-character* *source-position*))
...
(defun get-next-instruction ()
  ...
  (the (or null Instruction)
    (case *source-character*
      ...
      (#\0
        (read-niladic-instruction :no-op))
      (otherwise
        (incf *source-position*)
        NIL))))
...
(defclass Jump-Table ()
  ...
  (:documentation
    "The ``Jump-Table'' class establishes a bidirectional association
     betwixt the jump points in a Fumber program, mediated by
...
(defun build-jump-table-for (program)
  "Creates and returns a fresh ``Jump-Table'' which maps the Fumber
   PROGRAM's jump points by adminiculum of their zero-based indices."
  (declare (type Program program))
  ...
  ((cells
   ...
   (pointer
    ...
    :documentation "A mobile cursor which selects the currently active
                    cell."))
  (:documentation
    "The ``Tape'' class implements the Fumber program memory as a
...
(defclass Interpreter ()
  ((program
    ...
    :type          Program
    :documentation "The Fumber program instructions to execute.")
   (ip
    :initform      0
  ...
  (:documentation
    "The ``Interpreter'' class applies itself to the administration of
     actual efficacy to a Fumber program delivered in a sequence of
     instructions."))
...
(defun make-interpreter (program)
  "Creates and returns a fresh ``Interpreter'' instance dedicated to
   the Fumber PROGRAM's execution."
  (declare (type Program program))
  (the Interpreter
...
(defun program-is-completed-p (interpreter)
  "Determines whether the Fumber program consigned to the INTERPRETER's
   castaldy has been consumed in its entirety, returning on confirmation
...
(defun interpret-Fumber (code)
  "Interprets the piece of Fumber source CODE and returns no value."
...
;; Print "Hello, World!".
(interpret-Fumber "9H.9e.9l.9l.9o.9\",\".9\" \".9W.9o.9r.9l.9d.9!.0")

;;; -------------------------------------------------------

;; Repeating cat program which terminates on a "null character" input.
(interpret-Fumber "67568")
