...
;; language "AssemblerFuck++", invented by the Esolang user "Esolang1"
;; and presented on January 22nd, 2022, the immediate cleronomy of which
;; relates to the "AssemblerFuck" language, born on the stithy of the
;; user "Sesshomariu", and, as its successor, a reformulation of the
;; "brainfuck" programming language from Urban Mueller's efforts in a
;; form suggestive of an assembly language.
;; 
;; 
;; Concept
;; =======
;; The AssemblerFuck++ programming language enhances the AssemblerFuck
;; stock-father's dioristic mold of brainfuck in an assembly language
...
;;                  | literal.
;;   ..................................................................
;;   SET newValue   | Stores the {newValue} in the current cell.
;;       ********   | If the new value transcends the lower bourne of
;;                  | zero (0), the state wraps around along the same;
;;                  | if it exceeds the upper bourne of 255, its wraps
;;                  | around the same.
;;                  |--------------------------------------------------
;;                  | The {operand} must be a non-negative integer
;;                  | literal.
...
;;   ..................................................................
;;   UNTIL guard    | Repeatedly executes the {statements} until the
;;         *****    | current cell value equals the {guard}.
;;     statements   |--------------------------------------------------
...
;;                  | more statements.
;;   ..................................................................
;;   DO repetitions | If the {guard} does not equal zero (0), executes
;;      *********** | the {statements} a {guard} tally of times;
;;     statements   | otherwise executes the {statements} perpetually.
;;     **********   |--------------------------------------------------
;;   END            | The {guard} must be a non-negative integer
;;                  | literal. If equal to zero (0), an infinite loop
;;                  | is actuated; otherwise its value determines the
;;                  | number of repetitions.
;;                  |--------------------------------------------------
;;                  | The {statements} must be a sequence of zero or
;;                  | more statements.
;;   ..................................................................
;;   IF guard       | If the current cell value equals the {guard},
;;      *****       | executes the {statements} once; otherwise applies
...
;; 
;; Sources:
;;   [esolang2022AssemblerFuck++]
;;   The Esolang contributors, "AssemblerFuck++", September 17th, 2022
...
(flet ((register-identifier (name token)
  ...
  (register-identifier "DO"    (make-token :do    "DO"))
  ...
  (register-identifier "SET"   (make-token :set   "SET"))
...
(define-node-class Set-Node ()
  "The ``Set-Node'' class serves to encapsulate a \"SET\" instruction in
   conjunction with its operand."
  (value Integer-Node))
...
(define-node-class Until-Node ()
  "The ``Set-Node'' class serves to encapsulate an \"UNTIL\" instruction
   in conjunction with its guard and its statement block."
  (guard      Integer-Node)
  (statements Block-Node))

;;; -------------------------------------------------------

(define-node-class Do-Node ()
  "The ``Set-Node'' class serves to encapsulate a \"DO\" instruction in
   conjunction with its repetition count and its statement block."
  (repetitions Integer-Node)
  (statements  Block-Node))

;;; -------------------------------------------------------

(define-node-class If-Node ()
  "The ``Set-Node'' class serves to encapsulate an \"IF\" instruction in
   conjunction with its guard and its statement block."
  (guard      Integer-Node)
  (statements Block-Node))
...
(defun eat-current-token (parser)
  "Returns the PARSER's current token, while concomitantly querying and
   storing the successor from its underlying lexer."
  (declare (type Parser parser))
  (symbol-macrolet
      ((lexer         (the Lexer (parser-lexer         parser)))
       (current-token (the Token (parser-current-token parser))))
      ...
      (prog1
        (case (token-type current-token)
          ...
          (:until
            ...
            (let ((guard (parse-value parser)))
              (declare (type Integer-Node guard))
              (eat-newlines parser)
              (let ((statements (parse-statement-block parser)))
                (declare (type Block-Node statements))
                (eat-token parser :end)
                (make-until-node :guard guard :statements statements))))
          
          (:do
            (eat-token parser :do)
            (let ((repetitions (parse-value parser)))
              (declare (type Integer-Node repetitions))
              (eat-newlines parser)
              (let ((statements (parse-statement-block parser)))
                (declare (type Block-Node statements))
                (eat-token parser :end)
          ...
          (:if
            ...
            (let ((guard (parse-value parser)))
              ...
              (let ((statements (parse-statement-block parser)))
                (declare (type Block-Node statements))
                (eat-token parser :end)
                (make-if-node :guard guard :statements statements))))
...
(defgeneric move (interpreter target object)
  (:documentation
    "Applies the AssemblerFuck++ \"MOV\" instruction to the TARGET and
     the OBJECT, deployed in the INTERPRETER's context, and returns no
     value."))
...
(define-node-visit (Until-Node)
  (let ((guard
          (visit-node interpreter
            (until-node-guard node))))
    (declare (type non-negative-integer guard))
    (loop
      until
        (= (current-cell-value (get-memory interpreter))
           guard)
      do
        (visit-node interpreter
...
(define-node-visit (If-Node)
  (let ((guard
          (visit-node interpreter
            (if-node-guard node))))
    (declare (type non-negative-integer guard))
    (when (= (current-cell-value (get-memory interpreter))
             guard)
      (visit-node interpreter
...
;; Demonstrate the "SET" command by storing the value 65 in the incipial
;; cell and printing its ASCII character equivalent, the Latin letter
;; "A".
(interpret-AssemblerFuck++
  "SET 65
   MOV OUT, P")
